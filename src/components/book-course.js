...
const BookCourse = ({ isbutton, title, days, session, time, price, student, locationAt }) => {
  return (
    <View style={styles.courseWrapper}>
      ...
      <View style={styles.bookdtl}>
        <Text style={[styles.bookTxt, { color: themeColors.text_color_2, marginBottom: -2, fontWeight: '700' }]}>{price}</Text>
        {session && <Text style={[styles.bookTxt, { marginBottom: 5 }]}>{session}</Text>}
        {isbutton && <Button style={[globalStyle.tabBtn, globalStyle.activeTab]}>Book Now</Button>}
