...
import { View, Text, TextInput, ScrollView, TouchableWithoutFeedback, Keyboard, Image } from 'react-native';
...
const Login = ({ navigation, userLoginFun, setDefault, userData }) => {
  ...
  const handleLogin = values => {
    if (values?.number) userLoginFun(values.number);
  }
  useEffect(() => {
                ...
                {({ handleChange, handleSubmit, values, errors, touched, handleBlur }) => (
                  <View>
                    ...
                    <View style={{ marginTop: 30 }}>
                      ...
                      <Text style={globalStyle.termText_1}>Don’t have an account? <Text onPress={() => navigation.navigate('register')} style={globalStyle.termText_2}>Sign Up</Text></Text>
                    </View>
                  </View>
